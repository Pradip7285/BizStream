...
AUTHORIZED_USERS = [123456789, 7005979837, 6743744624]  # Replace with real IDs
...
# Zips folder and deletes original
...
async def invoice_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📅 Please send the date for the invoice in DD-MM-YYYY format."
    )

# Handle date input for invoice
...
async def handle_captcha(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if module == "invoice":
            invoice.scrape_invoice(driver, download_dir, session.get("date"))
        elif module == "stock":
        ...
        driver.quit()
        zip_path = zip_download_folder(download_dir)
...
# Smart router: date or captcha input
